...
public class AFNConstruct<T> {
    ...
    public Automata cerraduraKleene(Automata automataFN)
    {
        ...
        nuevoInicio.getTransiciones().add(new Transicion(nuevoInicio, anteriorInicio, Init.EPSILON));
        nuevoInicio.getTransiciones().add(new Transicion(nuevoInicio, nuevoFin, Init.EPSILON));
        
        // agregar transiciones desde el anterior estado final
        for (int i =0; i<anteriorFin.size();i++){
            anteriorFin.get(i).getTransiciones().add(new Transicion(anteriorFin.get(i), anteriorInicio,Init.EPSILON));
            anteriorFin.get(i).getTransiciones().add(new Transicion(anteriorFin.get(i), nuevoFin, Init.EPSILON));
        }
   ...
   public Automata concatenacion(Automata AFN1, Automata AFN2){
       ...
       Automata afn_concat = new Automata();
        ...
        for (i=0; i < AFN2.getEstados().size(); i++) {
            ...
            if (i == AFN2.getEstados().size()-1){
                ...
                {
                    tmp.setTransiciones(new Transicion((Estado) AFN2.getEstadosAceptacion().get(k),AFN1.getEstadoInicial(),Init.EPSILON));
                }
            }
    ...
    public Automata union(Automata AFN1, Automata AFN2){
        ...
        nuevoInicio.setTransiciones(new Transicion(nuevoInicio,AFN2.getEstadoInicial(),Init.EPSILON));
        ...
        for (int k =0; k<anteriorFin.size();k++)
            anteriorFin.get(k).getTransiciones().add(new Transicion(anteriorFin.get(k), nuevoFin, Init.EPSILON));
         // agregar transiciones desde el anterior AFN 2 al estado final
        for (int k =0; k<anteriorFin.size();k++)
            anteriorFin2.get(k).getTransiciones().add(new Transicion(anteriorFin2.get(k),nuevoFin,Init.EPSILON));
